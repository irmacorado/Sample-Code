{{ config(materialized='table', sort=["statecode","countyname","congressionaldistrict","statehousedistrict","statesenatedistrict"], dist = 'even') }}

with base as (
    select *
    from {{ ref('int_votefile_binned') }}
    where 
        deceased is FALSE
...
{% for race in ['white','black','latinx','aapi','nativeamerican','other']%}
    , {{race}} as (
        ...
        , {% if race in ('black','latinx','aapi','nativeamerican') %}'BIPOC'{% elif race == 'other' %}'Unknown / Other'{% elif race == 'white' %}'White'{% endif %} as bin_race_bipoc
        , bin_age 
        , bin_gender 
        , bin_partyreg
